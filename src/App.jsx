...
const seasonData = {
  ...
  2: {
    ...
    results: {
      ...
      // Result for match 8 is not available yet
    }
  }
};
...
function Predictor({ user, setView, selectedGameweek, setSelectedGameweek }) {
  ...
  const fixtures = seasonData[selectedGameweek].fixtures;
    ...
    const fetchPredictions = async () => {
      if (user) {
        ...
        if (!querySnapshot.empty) {
          ...
          setPredictions({}); // Clear predictions for the new week
        }
        setIsLoading(false);
      }
    };
...
function Leaderboard({ setView, selectedGameweek, setSelectedGameweek }) {
    ...
    useEffect(() => {
        const calculateScores = async () => {
            setIsLoading(true);
            const q = query(collection(db, "predictions"), where("gameweek", "==", selectedGameweek));
